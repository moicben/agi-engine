...
class AssistanteScraper {
  constructor(options = {}) {
    ...
    this.stats = {
      ...
      // Stats Supabase (gérées localement maintenant)
      supabaseStats: {
        created: 0,
        updated: 0,
        duplicates: 0,
  ...
  async saveLeadToSupabase(googleResult, phoneData, sourceInfo, verbose = true) {
    try {
      ...
      // Sauvegarder avec déduplication via contacts.js (modulaire)
      const { upsertLeadContact } = await import('../supabase/contacts.js');
      const result = await upsertLeadContact(leadData, verbose);
  ...
  async run() {
    ...
    if (this.saveToSupabase) {
      ...
      this.stats.supabaseStats = { created: 0, updated: 0, duplicates: 0, errors: 0 };
    }
  ...
  async runParallel() {
    ...
    // Les stats Supabase ont déjà été tenues à jour localement
  ...
  async getCompleteStats() {
    ...
    // Optionnel: pourrait interroger Supabase pour des stats globales ici via contacts.js
